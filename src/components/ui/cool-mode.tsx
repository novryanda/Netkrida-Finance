...
const applyParticleEffect = (
  ...
  const top = mouseY - (size ?? 0) / 2
  const left = mouseX - (size ?? 0) / 2
  ...
  circle.setAttributeNS(null, "cx", ((size ?? 0) / 2).toString())
  circle.setAttributeNS(null, "cy", ((size ?? 0) / 2).toString())
  circle.setAttributeNS(null, "r", ((size ?? 0) / 2).toString())
  ...
  circleSVG.setAttribute("width", (size ?? 0).toString())
  circleSVG.setAttribute("height", (size ?? 0).toString())
  ...
  const emojiSize = (size ?? 0) * fontSizeMultiplier
  ...
  const updateMousePosition = (e: MouseEvent | TouchEvent) => {
    if ("touches" in e) {
  mouseX = e.touches && e.touches[0] ? e.touches[0].clientX : 0
  mouseY = e.touches && e.touches[0] ? e.touches[0].clientY : 0
    } else {
      mouseX = e.clientX
      mouseY = e.clientY
    }
